#!/bin/bash
# Train a DFly draft model for Hy3 (multi-node).
# Reproduces: AngelSlim/Hy3-DFly-B8
#
...
#
# Prerequisites:
#   - Ray cluster running across nodes (ray start --head / ray start --address=...)
#   - Mooncake with RDMA configured (see docs/advanced_features/multi_node.md)
#   - Hy3 target model accessible on shared storage
#
# Usage:
#   ./examples/hy3-dfly/run.sh [EXTRA_ARGS...]
...
CONFIG_FILE="${1:-$ROOT_DIR/configs/vllm_hy3_dfly.yaml}"
if [[ -f "$CONFIG_FILE" ]]; then
    shift 1 || true
else
    CONFIG_FILE="$ROOT_DIR/configs/vllm_hy3_dfly.yaml"
fi
...
echo "=============================================="
echo "DFly Training (multi-node) — Hy3"
echo "Reproduces: AngelSlim/Hy3-DFly-B8"
echo "=============================================="
